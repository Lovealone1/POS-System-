...
from edit_cliente import FormularioEnvio
from tkinter import filedialog
import openpyxl

class CrudApp(ttk.Frame):
    def __init__(self, master):
        ...
        self.label_cc_nit.grid(row=1, column=0, sticky="e",pady=(10, 210))
        self.entry_cc_nit = tk.Entry(master, textvariable=self.cc_nit_var)
        self.entry_cc_nit.grid(row=1, column=1,pady=(10, 210))

        self.label_nombre = tk.Label(master, text="Nombre:")
        self.label_nombre.grid(row=1, column=0, sticky="e",pady=(50, 205))
        self.entry_nombre = tk.Entry(master, textvariable=self.nombre_var)
        self.entry_nombre.grid(row=1, column=1,pady=(50, 205))

        self.label_direccion = tk.Label(master, text="Dirección:")
        self.label_direccion.grid(row=1, column=0, sticky="e",pady=(50, 105))
        self.entry_direccion = tk.Entry(master, textvariable=self.direccion_var)
        self.entry_direccion.grid(row=1, column=1,pady=(50, 105))

        self.label_celular = tk.Label(master, text="Celular:")
        self.label_celular.grid(row=1, column=0, sticky="e",pady=(50, 155))
        self.entry_celular = tk.Entry(master, textvariable=self.celular_var)
        self.entry_celular.grid(row=1, column=1,pady=(50, 155))

        self.label_correo = tk.Label(master, text="Correo:")
        self.label_correo.grid(row=1, column=0, sticky="e",pady=(50, 50))
        self.entry_correo = tk.Entry(master, textvariable=self.correo_var)
        self.entry_correo.grid(row=1, column=1,pady=(50, 50))

        self.label_ciudad = tk.Label(master, text="Ciudad:")
        self.label_ciudad.grid(row=1, column=0, sticky="e",pady=(50, 0))
        self.entry_ciudad = tk.Entry(master, textvariable=self.ciudad_var)
        self.entry_ciudad.grid(row=1, column=1,pady=(50, 0))

        # Botones
        self.btn_agregar = tk.Button(master, text="Agregar", command=self.agregar)
        self.btn_agregar.grid(row=1, column=0, columnspan=2, pady=(120, 0))
        ...
        self.btn_actualizar = tk.Button(master, text="Actualizar", command=self.abrir_editar_cliente, bg="#CCFFFF")
        ...
        self.btn_bulk_insert = tk.Button(master, text="Cargar Clientes XL", command=self.bulk_insert_desde_excel_clientes)
    ...
    def cargar_datos_desde_db(self):
        try:
            ...
            # Ejecutar la sentencia SQL SELECT
    ...
    def agregar(self):
        ...
        # Verificar si ya existe un cliente con el mismo CC/NIT
        if self.existe_cliente(cc_nit_val):
            tk.messagebox.showerror("Error", "Ya existe un cliente con el mismo CC/NIT.")
            return
        ...
        self.insertar_cliente_en_db(cc_nit_val, nombre_val, direccion_val, celular_val, correo_val, ciudad_val)

        # Limpiar los campos de entrada después de agregar
        self.limpiar_campos()

    def existe_cliente(self, cc_nit):
        try:
            # Crear un cursor
            cursor = self.conexion.cursor()

            # Ejecutar la sentencia SQL SELECT para verificar si ya existe un cliente con el mismo CC/NIT
            cursor.execute("SELECT * FROM clientes WHERE cc_nit = ?", (cc_nit,))
            ...
            print(f"Error al verificar si existe el cliente en la base de datos: {e}")
            return False
    ...
    def abrir_editar_cliente(self):
        ...
        if not seleccion:
            tk.messagebox.showinfo("Información", "Por favor, seleccione un cliente.")
            return

        # Obtener los valores del cliente seleccionado
        id_cliente_seleccionado = self.lista.item(seleccion, 'values')[0]
        ...
        ciudad_val = self.lista.item(seleccion, 'values')[6]
        ...
        formulario_envio.id_var.set(id_cliente_seleccionado)
    ...
    def bulk_insert_desde_excel_clientes(self):
        ...
        try:
            ...
            for row_number, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
                ...
                if len(row) >= 6:
                    ...
                    self.insertar_cliente_en_db(cc_nit, nombre, direccion, celular, correo, ciudad)
    ...
    def insertar_cliente_en_db(self, cc_nit, nombre, direccion, celular, correo, ciudad):
        try:
            # Crear un cursor
            cursor = self.conexion.cursor()
            ...
            cursor.execute(
                "INSERT INTO clientes (cc_nit, nombre, direccion, celular, correo, ciudad) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    def eliminar(self, item):
        id_cliente_seleccionado = self.lista.item(item, 'values')[
            0]  # Obtener el valor de id_cliente del registro seleccionado
        if id_cliente_seleccionado:
            confirmar_eliminar = tk.messagebox.askyesno("Eliminar Registro",
                                                        f"¿Estás seguro de eliminar el registro con ID Cliente {id_cliente_seleccionado}?")
            if confirmar_eliminar:
                ...
                try:
                    ...
                    cursor.execute("DELETE FROM clientes WHERE id_cliente = ?", (id_cliente_seleccionado,))
